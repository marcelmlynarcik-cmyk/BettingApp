  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-xs font-black uppercase tracking-widest text-muted-foreground hover:bg-secondary transition-all"
      >
        <Settings2 className="h-4 w-4" />
        <span className="hidden md:inline">Športy/Ligy</span>
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-background/80 backdrop-blur-sm md:items-center">
          <div className="relative max-h-[90vh] w-full overflow-y-auto rounded-t-2xl border border-border bg-card p-4 shadow-2xl md:max-w-4xl md:rounded-xl md:p-6">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-black uppercase tracking-tight text-black md:text-xl">Správa športov a líg</h2>
                <p className="text-sm font-medium text-muted-foreground">Pridávanie, úprava a mazanie</p>
              </div>
              <button
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-border bg-secondary/30 p-3">
                <h3 className="mb-2 text-xs font-black uppercase tracking-widest text-muted-foreground">Športy</h3>
                <div className="mb-3 flex gap-2">
                  <input
                    value={newSportName}
                    onChange={(e) => setNewSportName(e.target.value)}
                    placeholder="Nový šport"
                    className="w-full rounded-lg border border-border bg-white px-3 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-primary"
                  />
                  <button
                    onClick={handleCreateSport}
                    disabled={isSaving}
                    className="rounded-lg bg-emerald-500 px-3 py-2 text-xs font-black uppercase tracking-wide text-white disabled:opacity-60"
                  >
                    Pridať
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {sortedSports.map((sport) => (
                    <div key={sport.id} className="rounded-lg border border-border bg-card px-2 py-2">
                      {editingSport?.id === sport.id ? (
                        <div className="flex gap-2">
                          <input
                            value={editingSport.name}
                            onChange={(e) => setEditingSport({ ...editingSport, name: e.target.value })}
                            className="w-full rounded-md border border-border bg-white px-2 py-1 text-sm font-semibold focus:outline-none focus:ring-1 focus:ring-primary"
                          />
                          <button
                            onClick={handleUpdateSport}
                            disabled={isSaving}
                            className="rounded-md bg-emerald-500 px-2 py-1 text-[10px] font-black uppercase text-white disabled:opacity-60"
                          >
                            Uložiť
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className="rounded-md border border-rose-300/50 p-1.5 text-rose-600 hover:bg-rose-500/10"
                            >
              ...
              <div className="rounded-xl border border-border bg-secondary/30 p-3">
                <h3 className="mb-2 text-xs font-black uppercase tracking-widest text-muted-foreground">Ligy</h3>
                ...
                <div className="space-y-2">
                  {sortedLeagues.map((league) => (
                    <div key={league.id} className="rounded-lg border border-border bg-card px-2 py-2">
